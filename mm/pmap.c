...
void mips_detect_memory()
{
	...
	basemem	= 0x04000000;	// 64MB = 2^26B -- 2^26 = (2^4)^6 * 2^2 = 0x0400_0000
	maxpa	= basemem;
	extmem	= 0;
	// Step 2: Calculate corresponding npage value.
	npage	= basemem >> PGSHIFT;	// 64MB/4KB = 16K -- 64M >> 2^12
...
//printf("&test_free:%lx, &test_pages[%d]:%lx\n", &test_free, i, &test_pages[i]);
...
void pageout(int va, int context)
{
	...
	p->pp_ref++; // It's a bug! it will be performed in page_insert()
